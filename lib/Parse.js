...
const Eat = (rules, type, token) => {
    ...
    return MakeError(() => `✗: unexpected token "${token.body}" of type ${Info(token.type)}`, token)
}

module.exports = (rules, type, tokens) => {
    return Eat(rules, type, TokenStream(tokens)).body
}
